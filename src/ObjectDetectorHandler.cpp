...
namespace {
std::vector<Object> nonMaxSupressDetections(const float confThreshold, const float nmsThreshold,
...
std::vector<Object> postprocess(const float confThreshold, const float nmsThreshold, const cv::Mat& frame,
                                const std::vector<cv::Mat>& outs, cv::dnn::Net& net,
                                const std::vector<std::string>& classNames, const std::vector<int>& classesToDetect) {
    std::vector<int> outLayers = net.getUnconnectedOutLayers();
    std::vector<Object> objectDetections;

    for (size_t i = 0; i < outs.size(); ++i) {
        // Network produces output blob with a shape NxC where N is a number of
        // detected objects and C is a number of classNames + 4 where the first 4
        ...
        for (int j = 0; j < outs[i].rows; ++j, data += outs[i].cols) {
            ...
            if (std::find(classesToDetect.begin(), classesToDetect.end(), classId) != classesToDetect.end()) {
                if (confidence >= confThreshold) {
                    ...
                    objectDetections.push_back(
                        {classId, classNames[classId], confidence, cv::Rect(left, top, width, height)});
                }
            }
        }
    }
